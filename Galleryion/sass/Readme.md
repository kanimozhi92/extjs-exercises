...
folder are mapped to the application's root namespace, 'Galleryion'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "Galleryion"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'Galleryion' in the same way
as `"Galleryion/sass/src"`.
